...
**First action every loop:** read `.planning/page-forge/softree-agentic-exact/loop-state.json`.  
...
| --- | --- | --- |
| Hero BG image | `SOFTREE_AGENTIC_HERO_BG_IMAGE` in `src/components/softree-agentic-exact/softreeAgenticHeroAssets.ts` | `#meet-artemis .k2-bg img` or equivalent |

**Rules:**

- Import `SOFTREE_AGENTIC_HERO_BG_IMAGE` in `SoftreeAgenticLoader.tsx` — never duplicate the URL string.
...
Dispatch `softree-agentic-intro-complete` + add `.softree-agentic-intro-complete` on shell.
...
| Scroll lock | `html.softree-agentic-k2-loader-running { overflow: hidden }` only during loader |
...
| File | Role |
| --- | --- |
| `src/components/softree-agentic-exact/softreeAgenticHeroAssets.ts` | Single source for bg URL |
| `src/components/softree-agentic-exact/SoftreeAgenticLoader.tsx` | Loader markup, portal, shared bg img |
| `src/components/softree-agentic-exact/k2-loader.css` | Step animations, perspective stage, grain, bar |
| `src/components/softree-agentic-exact/k2LoaderRuntime.ts` | Step machine, hard stop, session skip |
| `src/components/softree-agentic-exact/k2CinematicHandoff.ts` | GSAP handoff timeline (primary edit surface) |
| `src/components/softree-agentic-exact/SoftreeAgenticHeroSection.tsx` | `data-flip-target="loader"`, hero bg DOM |
| `src/components/softree-agentic-exact/SoftreeAgenticPage.tsx` | Wires loader + hero; no root layout |
| `src/components/softree-agentic-exact/softree-agentic-page-fix.css` | Hero visibility fixes post-intro |

**Do not use** `SoftreeAgenticIntroTransition.tsx` for this loop — K2 loader is canonical for `/agentic-ai-platform`.
...
Read current: k2CinematicHandoff.ts, k2-loader.css, SoftreeAgenticLoader.tsx, k2LoaderRuntime.ts.

Implement or refine:
1. Shared SOFTREE_AGENTIC_HERO_BG_IMAGE in loader + hero (no URL drift).
...
Replay URL: http://localhost:3000/agentic-ai-platform?replay-loader=1
...
Save to .planning/page-forge/softree-agentic-exact/handoff-loop-<n>/.
Compare hero settled frame to reference https://www.softreetechnology.com/ai-agent-platform hero.
...
| 2 | Route | `curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/agentic-ai-platform` | 200 |
| 3 | Shared asset | grep: loader imports `SOFTREE_AGENTIC_HERO_BG_IMAGE` | single import |
...
**Fresh intro session key (Barba overlay — separate):** `softree-agentic-intro-v1` in sessionStorage — do not conflate with K2 loader.
...
```
Run loader→hero cinematic handoff loop on softree-agentic-exact.
