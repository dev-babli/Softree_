...
`softree-agentic-page-fix.css` set `.k2-bg { z-index: -1 }` to keep text above the image. Without an isolated stacking context on `#meet-artemis`, the negative z-index painted the background **behind** the black `.softree-agentic-shell`, so the WebP never showed.

Secondary: handoff used `opacity: unset` on `.k2-bg` during `softree-agentic-k2-handoff-running`, and GSAP could leave inline `opacity: 0` on `.k2-bg` after the cinematic timeline.

## Fix (shipped)

| File | Change |
| --- | --- |
| `softree-agentic-page-fix.css` | `#meet-artemis { isolation: isolate; overflow: hidden }`; `.k2-bg { z-index: 0 }`; `.k2-overlay { z-index: 1 }`; containers `{ z-index: 2 }`; force `opacity: 1` during handoff + intro-complete |
...
(async()=>{
  ...
  await page.goto('http://localhost:3001/agentic-ai-platform?replay-loader=1',{waitUntil:'domcontentloaded'});
  await page.waitForFunction(()=>document.querySelector('.softree-agentic-shell')?.classList.contains('softree-agentic-intro-complete'),{timeout:25000});
