...
| File | Change |
| --- | --- |
| `kore-light-theme-fix.css` | Light-theme visibility, agents bg stacking, orbit buttons, scroll-reveal overrides |
| `lightThemeReveal.ts` | `revealLightThemeSections()` — adds `.on`, clears inline opacity/visibility on scroll/stagger nodes |
| `KoreAiExactPage.tsx` | Calls `revealLightThemeSections()` in scroll-reveal `bindReveal` |
| `KoreAgentsSection.tsx` | `.on` on `.k2-agents-panel` / `.k2-agents-content`; reveal on tab change |
| `KoreAiProgrammableSection.tsx` | Raw Kore HTML; reveal on mount |
| `KorePillarsSection.tsx` | Raw Kore HTML; reveal on mount |
| `koreHtmlCopy.ts` | `patchMiddleSectionHtml` → identity (Kore copy restored) |
