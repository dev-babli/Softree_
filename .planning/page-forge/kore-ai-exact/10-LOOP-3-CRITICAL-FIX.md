...
| File | Fix |
| --- | --- |
| `KoreBarbaIntroTransition.tsx` | Shorter Softree intro, 6s hard stop, session skip, reliable `completeIntro()` |
| `kore-ai-page-fix.css` | Hero bg always visible; intro-complete unhides hero; middle reveal safety |
| `KoreAiExactPage.tsx` | Page-wide reveal observer; body bg reset; fix CSS import |
| `koreHtmlCopy.ts` | Regex/HTML-aware Softree copy for programmable + pillars |
| `KoreHeroSection.tsx` | Softree hero copy + `/contact` CTA |
| `KoreEnterpriseOutcomesSection.tsx` | Softree outcomes copy |
| `KoreAgentsSection.tsx` | Softree agents copy |
