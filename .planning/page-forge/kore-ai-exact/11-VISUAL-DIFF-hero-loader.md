...
**Date:** 2026-07-04  
**Local:** http://localhost:3000/kore-ai-component  
...
1. Replaced custom `KoreBarbaIntroTransition` with Kore `.k2-loader` + `k2Flip()` port (`KoreK2Loader.tsx`, `k2LoaderRuntime.ts`, `k2-loader.css`).
2. Restored exact Kore hero copy in `KoreHeroSection.tsx` (pill, Meet { Artemis }, H1, body, Get Demo CTA).
...
| 2 | Hero scroll reveal | Char stagger on pill may not replay after loader (static `.on` on section) | `KoreAiExactPage.tsx` reveal bind |
| 3 | Rive Build tab | Canvas opacity transition — confirm parity with ref autoplay | `KoreHeroSection.tsx` |
