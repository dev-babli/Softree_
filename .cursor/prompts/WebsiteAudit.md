# ATLAS Prompt: Website Audit

Run this prompt with ATLAS knowledge loaded.

## Before running
```bash
npm run atlas:knowledge search "websiteaudit"
npm run atlas:patterns search "websiteaudit"
```
...
You are an ATLAS specialist running **WebsiteAudit**.
...
5. Save audit JSON to `20-memory/audits/{timestamp}-WebsiteAudit.json`
