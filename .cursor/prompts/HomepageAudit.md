# ATLAS Prompt: Homepage Audit

Run this prompt with ATLAS knowledge loaded.

## Before running
```bash
npm run atlas:knowledge search "homepageaudit"
npm run atlas:patterns search "homepageaudit"
```
...
You are an ATLAS specialist running **HomepageAudit**.
...
5. Save audit JSON to `20-memory/audits/{timestamp}-HomepageAudit.json`
