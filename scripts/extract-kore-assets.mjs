 ...
 *
 * Reads `public/softree-source-sections.html` and extracts every distinct asset
 * URL referenced by:
 *   - <img src>, <img srcset>
 ...
 *                      .riv fallback into public/softree-marketing/<section>/<file>.<ext>
 *                      (and public/softree-marketing/rive/<file>.riv). Idempotent: skips
 ...
 *
 * Used as the input for `src/components/softree-marketing-ui/assets.ts` (Requirements 24.1,
 * 24.2, 24.3, 24.4, 24.5).
 */
...
function aggregate(html, lineOf) {
    ...
    for (const [url, { firstOffset, sources }] of byUrl) {
        ...
        if (ext === 'riv' && kind === 'cdn-passthrough') {
            ...
            entry.fallback = {
                kind: 'local',
                url: `/softree-marketing/rive/${kebab}`,
                basename: kebab,
                ext,
            };
        }
...
//
// Writes `src/components/softree-marketing-ui/assets.ts` from the inventory. Idempotent: same
...
function emitAssetsTs(inventory) {
    const header = `/**
 * Asset_Manifest for the Softree homepage clone.
 *
 * SOURCE OF TRUTH: scripts/.kore-asset-inventory.json
 *   - generated by scripts/extract-kore-assets.mjs from public/softree-source-sections.html
 ...
 * \`fallback\` field referencing a Local_Asset under \`/softree-marketing/rive/\` (Requirement
 * 24.4). The local fallback files are staged under \`public/softree-marketing/rive/\` by
...
export interface LocalAsset {
    readonly kind: 'local';
    readonly url: \`/softree-marketing/\${string}\`;
}
