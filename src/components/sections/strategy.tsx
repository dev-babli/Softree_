      ...
      <div className="max-w-7xl mx-auto space-y-28">
        {[data.slice(0, 3), data.slice(3, 6)].map((row, rowIndex) => (
          <div key={rowIndex} className="relative grid md:grid-cols-3 gap-12">
            {/* Horizontal Line */}
            <div className="hidden md:block absolute top-6 left-0 w-full h-[2px] bg-gray-700"></div>

            {row.map((item, index) => (
              <div key={index} className="relative text-center">
                {/* Number Pill (CENTERED ON LINE) */}
                <div
                  className="absolute top-6 left-1/2 -translate-x-1/2 -translate-y-1/2 
                       bg-black border border-gray-600 px-4 py-1 rounded-full text-sm z-10"
                >
                  {item.id}
                </div>
                ...
                <div className="mt-14 mb-6 space-y-3">
                  ...
                  <div className="flex justify-center">
                    <div className="w-16 h-[2px] bg-gradient-to-r from-transparent via-gray-500 to-transparent"></div>
                  </div>
                </div>
