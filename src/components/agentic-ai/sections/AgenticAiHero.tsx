...
import { agenticHero, heroTestimonials } from "../data"
  ...
  useGSAP(
    () => {
      ...
      const runIntro = () => {
        ...
        tl.from(".ai-hero-fade", {
          y: 32,
          opacity: 0,
          duration: DUR.section,
          stagger: STAGGER.default,
        }).from(".ai-hero-stage", { scale: 0.98, opacity: 0, duration: DUR.cinematic }, "-=0.45")
      }

      if (document.documentElement.classList.contains("agentic-ai-ready")) {
        runIntro()
        return
      }

      const onReady = () => runIntro()
      window.addEventListener("agentic-ai:ready", onReady, { once: true })
      return () => window.removeEventListener("agentic-ai:ready", onReady)
    },
    { scope: containerRef },
  )

  const review = heroTestimonials[reviewIdx]
  const { pill, headline, headlineAccent, subhead, stats } = agenticHero
    ...
    <section ref={containerRef} className="relative isolate w-full overflow-hidden bg-white">
      <div className="mx-auto w-full max-w-[1440px] px-4 pb-12 pt-4 sm:px-6 md:px-8 md:pb-16 lg:px-10">
        <SectionHeader
          ...
          body={subhead}
          className="ai-hero-fade mb-10 max-w-4xl"
        />

        <div className="ai-hero-fade mb-8 flex flex-wrap items-center gap-4">
          <LetsTalkButton />
          <Link
        ...
        <div className="ai-hero-stage relative mt-4 overflow-hidden rounded-3xl bg-[#0a0a1a] shadow-[0_20px_60px_-12px_rgba(0,0,0,0.25)] md:rounded-[40px]">
          <div className="relative aspect-[16/9] min-h-[320px] w-full md:min-h-[420px]">
            <Image
              src={HERO_IMG}
              alt="Enterprise team building agentic AI on the Microsoft stack"
              fill
            ...
            <div className="relative z-10 flex h-full flex-col justify-end p-6 sm:p-8 md:p-10 lg:p-12">
              ...
              >
                Agentic AI on{" "}
                <span className="relative inline-block min-w-[200px] align-baseline sm:min-w-[280px]">
              ...
              <p className="mt-4 max-w-xl text-sm leading-relaxed text-white/75 md:text-base">
                Production-grade agents with governance your IT team can audit, not demos that stall in pilot.
              </p>
            </div>
          </div>
        </div>

        <div className="ai-hero-fade mt-10 grid gap-4 sm:grid-cols-3 lg:mt-12">
          {stats.map((item) => (
            <SpotlightCard
              key={item.label}
              color="rgba(255, 88, 18, 0.35)"
              className="rounded-2xl border border-[var(--legacy-111)]/[0.08] bg-white p-6 shadow-[inset_0_1px_0_rgba(255,255,255,1),0_20px_50px_-28px_rgba(17,17,17,0.12)]"
            >
              <p className="text-3xl font-bold tabular-nums tracking-tight text-[#0a0a1a] md:text-4xl">{item.value}</p>
        ...
        <SpotlightCard
          color="rgba(255, 88, 18, 0.35)"
          className="ai-hero-fade mt-6 rounded-2xl border border-[var(--legacy-111)]/[0.08] bg-[#F8F9FC] p-6 md:p-8"
        >
