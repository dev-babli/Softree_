 ...
 *
 * Unit tests for the `SoftreeSwiperWrapper` primitive (task 3.6).
 *
...
import { SoftreeSwiperWrapper } from '../softree-swiper-wrapper';
...
describe('SoftreeSwiperWrapper', () => {
    ...
    it('wraps a single (non-array) slide node in one .swiper-slide', () => {
        const { container } = render(
            <SoftreeSwiperWrapper config={EMPTY_CONFIG} slides={<span>solo</span>} />,
        );
    ...
    it('omits default navigation and pagination elements by default', () => {
        const { container } = render(
            <SoftreeSwiperWrapper config={EMPTY_CONFIG} slides={[<span key="a">A</span>]} />,
        );
