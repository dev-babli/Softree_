 ...
 *
 * Unit tests for the `SoftreeButtonDot` primitive (task 3.1).
 *
...
import { SoftreeButtonDot } from '../softree-button-dot';

afterEach(() => {
    cleanup();
});

describe('SoftreeButtonDot', () => {
    it('renders the label inside the source `.text-style-1line` node', () => {
        render(<SoftreeButtonDot label="Get a demo" />);
    ...
    it('mirrors the Source_Document dot + underline DOM structure and class names', () => {
        const { container } = render(<SoftreeButtonDot label="Learn more" />);
    ...
    it('preserves the Webflow `w-inline-block` class token', () => {
        const { container } = render(<SoftreeButtonDot label="Register" />);
        expect(
    ...
    it('renders a <button type="button"> when no href is provided', () => {
        const { container } = render(<SoftreeButtonDot label="Open" />);
    ...
    it('forwards the click handler', () => {
        const onClick = vi.fn();
        render(<SoftreeButtonDot label="Submit" onClick={onClick} />);
        fireEvent.click(screen.getByText('Submit'));
        expect(onClick).toHaveBeenCalledTimes(1);
    });

    it('defaults to the primary variant (no is-ghost modifier attribute)', () => {
        const { container } = render(<SoftreeButtonDot label="Primary" />);
    ...
    it('merges a caller-supplied className after the source class tokens', () => {
        const { container } = render(
            <SoftreeButtonDot label="Custom" className="for-nav" />,
        );
