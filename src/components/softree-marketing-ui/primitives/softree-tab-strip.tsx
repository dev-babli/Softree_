...
/**
 * `SoftreeTabStrip` — the shared, accessible tab-list primitive that powers every
 * tabbed section of the Softree homepage: Industry_Tabs (Req 7.3, 7.4),
 ...
 * on the panel side in {@link SoftreeTabPanel}, which derives its ids from the
 * SAME {@link softreeTabDomId} / {@link softreePanelDomId} helpers exported here so
 * the tab↔panel pairing is guaranteed one-to-one.
 *
 * CONTROLLED COMPONENT (by design — see design.md `SoftreeTabStrip` and the
...
/** Render-state passed to the optional {@link SoftreeTabStripProps.renderTab}. */
export interface KoreTabRenderState {
    /** `true` when this tab is the currently active one. */
    readonly active: boolean;
}

export interface SoftreeTabStripProps<T extends string> {
     ...
     * {@link SoftreeTabPanel}s MUST be given the same `idBase` so each tab's
...
/**
 * Compose the DOM id for a tab element. Shared with {@link SoftreeTabPanel} (via
 ...
 */
export function softreeTabDomId(idBase: string, tabId: string): string {
    return `${idBase}-tab-${tabId}`;
}

/**
 * Compose the DOM id for a tab panel element. Shared with {@link SoftreeTabPanel}
 * (its `id`) and with each tab's `aria-controls`.
 */
export function softreePanelDomId(idBase: string, tabId: string): string {
...
export function SoftreeTabStrip<T extends string>({
    tabs,
    active,
    onActivate,
    idBase,
    ...
    renderTab,
}: SoftreeTabStripProps<T>): React.JSX.Element {
            ...
            {tabs.map((tab) => {
                ...
                return (
                    <button
                        ...
                        type="button"
                        role="tab"
                        id={softreeTabDomId(idBase, tab.id)}
                        aria-selected={isActive}
                        aria-controls={softreePanelDomId(idBase, tab.id)}
