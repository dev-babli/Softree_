...
export function SoftreeAgenticProgrammableSection() {
  ...
  const html = useMemo(() => {
    const section = SOFTREE_AGENTIC_SECTIONS.find((item) => item.name === "SoftreeAgenticProgrammableSection")
    return applySoftreeSectionHtml("SoftreeAgenticProgrammableSection", section?.html ?? "")
  }, [])
