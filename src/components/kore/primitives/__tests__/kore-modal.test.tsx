 ...
 *
 * Unit tests for the `KoreModal` primitive (task 3.5).
 *
...
import { KoreModal } from '../kore-modal';
...
describe('KoreModal', () => {
    it('renders nothing while closed', () => {
        render(
            <KoreModal open={false} onClose={() => { }} ariaLabel="Demo">
    ...
    it('renders a dialog with aria-modal and aria-labelledby when open (Req 25.6)', async () => {
        render(
            <KoreModal open onClose={() => { }} labelledById="heading-1">
    ...
    it('falls back to aria-label when no labelledById is provided', async () => {
        render(
            <KoreModal open onClose={() => { }} ariaLabel="Exit modal">
                <button type="button">Close</button>
            </KoreModal>,
        );
        await flushFrames();
    ...
    it('applies `lenis-stopped` to <html> while open (Req 13.4, 15.3)', async () => {
        render(
            <KoreModal open onClose={() => { }} ariaLabel="Demo">
                <button type="button">Close</button>
            </KoreModal>,
        );
        await flushFrames();
        ...
        function Harness() {
            ...
            return (
                <>
                    ...
                    <KoreModal open={open} onClose={() => setOpen(false)} ariaLabel="Demo">
                        <button type="button">Inside</button>
                    </KoreModal>
                </>
            );
    ...
    it('invokes onClose when Escape is pressed (Req 15.4)', async () => {
        const onClose = vi.fn();
        render(
            <KoreModal open onClose={onClose} ariaLabel="Demo">
                <button type="button">Close</button>
            </KoreModal>,
        );
        await flushFrames();
    ...
    it('invokes onClose on backdrop click by default', async () => {
        const onClose = vi.fn();
        render(
            <KoreModal open onClose={onClose} ariaLabel="Demo" backdropClassName="kore-backdrop">
                <button type="button">Close</button>
            </KoreModal>,
        );
        await flushFrames();
    ...
    it('preserves the Source_Document `modal` and `open` class tokens', async () => {
        render(
            <KoreModal open onClose={() => { }} ariaLabel="Demo" className="enterprise">
                <button type="button">Close</button>
            </KoreModal>,
        );
        await flushFrames();
