 ...
 *
 * Unit tests for the `KoreRiveCanvas` primitive (task 3.7).
 *
...
import { KoreRiveCanvas } from '../kore-rive-canvas';
...
const poster = {
    url: '/kore/rive/pre-built-applications.png',
...
describe('KoreRiveCanvas', () => {
    it('renders the poster <img> immediately with its src and alt (Req 6.4, 6.9)', () => {
        render(<KoreRiveCanvas riveSrc="https://cdn/x.riv" poster={poster} />);
    ...
    it('renders a <canvas> overlay carrying the source `rive-canvas` class token', () => {
        const { container } = render(
            <KoreRiveCanvas riveSrc="https://cdn/x.riv" poster={poster} />,
        );
    ...
    it('forwards reducedMotion + riveSrc into useRiveBlock', () => {
        mockUseReducedMotion.mockReturnValue(true);
        render(<KoreRiveCanvas riveSrc="https://cdn/special.riv" poster={poster} />);
