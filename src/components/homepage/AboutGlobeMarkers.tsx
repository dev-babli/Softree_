...
@media (max-width:768px){
  .globe-heading{font-size:32px; margin-bottom: 24px; text-align: left; width: 100%;}
  ...
  .switcher .store-name{min-width: 85px; font-size: 12px;}
  .caption{text-align: left; max-width: 100%;}
}
`;

export default function AboutGlobeMarkers({ variant = 'dark' }: { variant?: 'light' | 'dark' }) {
  const heading = 'Where our clients are';
  const locations = DEFAULT_LOCATIONS;
  const legs = DEFAULT_LEGS;
  const caption = 'Global reach. Local expertise. Delivering engineering capabilities across markets.';
  const storesLabel = '13 countries served';
  ...
  const drawGlobe = useCallback(() => {
    ...
    // 1. Opaque base to prevent page background from showing through
    ...
    baseGrad.addColorStop(0, "#1c1510");
    baseGrad.addColorStop(1, "#0a0908");
    ...
    // 2. Original sphere background shadow & subtle glow overlay
      ...
      <section className="globe-section">
        <div className="globe-container">
          <div className="globe-badge">
            <span className="globe-badge-dot"></span>
            <span className="globe-badge-text">GLOBAL REACH. LOCAL UNDERSTANDING.</span>
          </div>
          {heading && (
