...
function applyShellRegex(html: string): string {
  return html
    ...
    .replace(
      /https:\/\/cdn\.prod\.website-files\.com\/[^"']*Kore%20Logo%20White\.svg/g,
...
const NAV_FOOTER: ReplacePair[] = [
  ...
  ['href="/ai-agent-platform"', 'href="/kore-ai-component"'],
  ...
  ["© 2026 Kore.ai Inc.", "© 2026 Softree Technology"],
]
...
const SHELL: ReplacePair[] = [
  ...
  ["Agent Platform {&nbsp;Artemis&nbsp;}", "Agentic AI · Softree"],
  ["About Kore.ai", "About Softree"],
  ["Kore ai white logo in display.", "Softree Technology logo."],
  ["© 2026 Kore.ai Inc.", "© 2026 Softree Technology"],
  ["Agent Marketplace", "Case studies"],
...
export function applySoftreeSectionHtml(sectionName: string, html: string): string {
  ...
  switch (sectionName) {
    case "KoreAiProgrammableSection":
      out = chainReplace(out, PROGRAMMABLE)
      break
    case "KorePillarsSection":
      out = chainReplace(out, PILLARS)
      break
    case "KoreBuildScaleOptimizeSection":
      out = chainReplace(out, BUILD_SCALE)
      break
    case "KoreSection7Section":
      out = chainReplace(out, DEMO_VIDEO)
      break
    case "KoreK2SectionScrollTabsSection":
      out = chainReplace(out, SCROLL_TAB_BODIES)
      break
    case "KoreGetStartedSection":
