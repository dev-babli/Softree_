...
export function KoreDemoVideoSection() {
  const sectionRef = useRef<HTMLElement>(null)
  const overlayRef = useRef<HTMLDivElement>(null)
  const section = useMemo(
    () => KORE_AI_SECTIONS.find((item) => item.name === "KoreSection7Section"),
    [],
  )
  ...
  useEffect(() => {
    ...
    const player = win.Vimeo?.Player ? new win.Vimeo.Player(iframe) : null
    const state = {
      playing: false,
      activated: false,
      meetingLoaded: false,
  ...
  return (
    <>
      <section ref={sectionRef} className="k2-section" dangerouslySetInnerHTML={{ __html: section.html }} />
      <div ref={overlayRef} className="bamform-overlay" id="bamform-overlay" role="dialog" aria-modal="true" aria-label="Book a demo meeting">
