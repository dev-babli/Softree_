...
import "./kore-scroll-tabs-fix.css"
...
function mountKoreScrollTabs(sectionNode: HTMLElement): () => void {
...
export function KoreScrollTabsSection() {
  ...
  const section = useMemo(() => {
    const raw = KORE_AI_SECTIONS.find((item) => item.name === "KoreK2SectionScrollTabsSection")
    if (!raw) return null
    return { ...raw, html: applySoftreeSectionHtml("KoreK2SectionScrollTabsSection", raw.html) }
  }, [])
  ...
  useEffect(() => {
    ...
    const mount = () => {
      unbind()
      win.ScrollTrigger?.refresh()
      unbind = mountKoreScrollTabs(sectionNode)
      win.ScrollTrigger?.refresh()
    }
    ...
    if (document.documentElement.classList.contains("kore-ai-intro-complete")) {
      mount()
    } else {
      window.addEventListener("kore-ai-intro-complete", onIntroComplete, { once: true })
    }
