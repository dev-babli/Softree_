...
export function getPublishChecklist(
  ...
  if (dataset === 'production' && (isCaseStudy || isPost)) {
    ...
    items.push({
      id: 'websiteStatus',
      label: 'Website status → Published',
      pass: !doc.status || doc.status === 'published',
      tab: 'Publish & SEO',
      hint: 'Use ⋯ → Publish (sets automatically) or Mark live on website',
    })
  }
...
/** Single-document readiness score for content score panel (0–100). */
export function publishReadinessPercent(
  type: string,
  doc: PublishReadinessDoc | null | undefined,
): number {
  const checklist = getPublishChecklist(type, doc)
