...
import { getAutocompleteFields, getRewriteFields } from "./fieldRegistry"
import { SOFTREE_STYLE_CONTEXT } from "./constants"
import { studioInstructionTemplates } from "./instructionTemplates"
import { suggestSlugFromTitle } from "@/cms/lib/studio/slugUtils"

const AI_CONTEXT_QUERY = `*[_type == "aiContext" && isDefault == true][0].context`

const AUTOCOMPLETE_FIELDS = getAutocompleteFields()
const REWRITE_FIELDS = getRewriteFields()
...
async function autocompleteField(
  ...
  action: "autocomplete" | "rewrite" = "autocomplete",
): Promise<void> {
...
export function useSoftreeAssistFieldActions(props: AssistFieldActionProps) {
  ...
  return useMemo(() => {
    ...
    if (actionType === "field" && AUTOCOMPLETE_FIELDS.has(fieldName)) {
      const actions = [
        defineAssistFieldAction({
          ...
          onAction: async () => {
            try {
              await autocompleteField(props, fieldName, toast, client, "autocomplete")
            } catch (err) {
              toast.push({
                status: "error",
      ...
      if (REWRITE_FIELDS.has(fieldName)) {
        actions.push(
          defineAssistFieldAction({
            ...
            onAction: async () => {
              try {
                ...
                await autocompleteField(props, fieldName, toast, client, "rewrite")
                if (!current) return
              } catch (err) {
                toast.push({
                  status: "error",
                  title: "Rewrite failed",
                  description: err instanceof Error ? err.message : "Could not rewrite field",
                })
              }
            },
          }),
    ...
    if (actionType === "field" && fieldName === "slug") {
      return [
        defineAssistFieldAction({
          title: "Suggest slug from title",
          icon: SparklesIcon,
          onAction: async () => {
            try {
              const doc = getDocumentValue?.() ?? {}
              const title = (doc.title as string) || ""
              const slug = await suggestSlugFromTitle(title)
              ...
              toast.push({ status: "success", title: "Slug suggested", description: slug })
            } catch (err) {
              toast.push({
                status: "error",
                title: "Slug suggestion failed",
