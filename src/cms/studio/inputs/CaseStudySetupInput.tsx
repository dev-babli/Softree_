...
import { set, useFormCallbacks, type StringInputProps } from 'sanity'
...
import { effectiveCaseStudyCategory, getLegacyCaseStudyFieldPatches } from '@/cms/lib/studio/caseStudyLegacy'
...
} from '@/cms/lib/studio/caseStudyEditorGuide'
...
function useLegacyCaseStudyContext() {
  const industry = useFormValue(['industry']) as string | undefined
  const useCase = useFormValue(['useCase']) as string | undefined
  ...
  const heroHeadline = useFormValue(['heroHeadline']) as string | undefined

  return { industry, useCase, detailLayout, title, client, headerTitle, heroHeadline }
}
...
export default function CaseStudySetupInput(props: StringInputProps) {
  ...
  const { onChange: onDocumentPatch } = useFormCallbacks()
  const legacyContext = useLegacyCaseStudyContext()

  const title = legacyContext.title
  const slug = useFormValue(['slug', 'current']) as string | undefined
  const detailLayout = legacyContext.detailLayout || EDITOR_PAGE_FORMAT.value

  const selectedCategory = isCaseStudyCategory(props.value as string)
    ? (props.value as CaseStudyCategoryKey)
    : undefined

  const detectedCategory = effectiveCaseStudyCategory({
    category: props.value as string | undefined,
    industry: legacyContext.industry,
    useCase: legacyContext.useCase,
    detailLayout: legacyContext.detailLayout,
    title: legacyContext.title,
    client: legacyContext.client,
  })

  const legacyPatches = getLegacyCaseStudyFieldPatches({
    category: props.value as string | undefined,
    industry: legacyContext.industry,
    useCase: legacyContext.useCase,
    detailLayout: legacyContext.detailLayout,
    title: legacyContext.title,
    client: legacyContext.client,
    headerTitle: legacyContext.headerTitle,
    heroHeadline: legacyContext.heroHeadline,
  })
  ...
  const selectLayout = useCallback(
    (value: string) => {
      onDocumentPatch([set(value, ['detailLayout'])])
    },
    [onDocumentPatch],
  )

  const applyLegacyPatches = useCallback(() => {
    const events = Object.entries(legacyPatches).map(([path, value]) =>
      set(value, [path]),
    )
    if (events.length === 0) return
    onDocumentPatch(events)
  }, [legacyPatches, onDocumentPatch])
    ...
    <Stack space={5}>
      <Card padding={4} radius={3} tone="primary" border>
        <Stack space={4}>
          ...
          <SetupProgress
            categoryDone={!!selectedCategory || !!detectedCategory}
      ...
      <Stack space={3}>
        ...
        {!selectedCategory && detectedCategory ? (
          <Card padding={3} radius={2} tone="caution" border>
            <Stack space={3}>
              <Text size={1}>
                Legacy story — detected category{' '}
                <strong>{CASE_STUDY_CATEGORY_CONFIG[detectedCategory].title}</strong> from use case /
                layout fields. Save a canonical category below or sync all legacy fields.
              </Text>
              <Flex gap={2} wrap="wrap">
                <Button
                  fontSize={1}
                  tone="primary"
                  text={`Use ${CASE_STUDY_CATEGORY_CONFIG[detectedCategory].title}`}
                  onClick={() => selectCategory(detectedCategory)}
                />
                {Object.keys(legacyPatches).length > 0 ? (
                  <Button
                    fontSize={1}
                    mode="ghost"
                    text="Sync legacy fields (category + header title)"
                    onClick={applyLegacyPatches}
                  />
                ) : null}
              </Flex>
            </Stack>
