...
// ── ULTRA-THIN SVG ICONS (Phosphor-style strokes) ──
const Icons = {
  Globe: () => (
...
export default function AboutHeroElite() {
  ...
  useEffect(() => {
    ...
    const elements = ctx.querySelectorAll("[data-animate]")
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex min-h-[100dvh] max-w-[1400px] flex-col px-6 pt-32 md:px-12 lg:flex-row lg:items-center lg:gap-20 lg:px-16 lg:pt-0">
        ...
        <div className="flex flex-1 flex-col justify-center">
          ...
          >
            {/* Primary CTA - Double-bezel button */}
            <Link
              href="/contact"
              className="group relative inline-flex items-center gap-3 rounded-full bg-white px-6 py-3.5 text-sm font-medium text-black transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] active:scale-[0.98]"
            >
              Start Your Project
              {/* Nested circular icon container */}
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-black/5 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:scale-105">
                <span className="text-black transition-transform duration-500 group-hover:rotate-45">
                  <Icons.ArrowUp />
                </span>
              </span>
            </Link>
            ...
            <Link
              href="/services"
              className="group inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/[0.02] px-6 py-3.5 text-sm font-medium text-white backdrop-blur-sm transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] hover:border-white/20 hover:bg-white/[0.04]"
            >
              Explore Services
              <span className="text-white/40 transition-all duration-500 group-hover:text-white">
                <Icons.ArrowUp />
              </span>
            </Link>
          </motion.div>
          ...
          >
            {STATS.map((stat, i) => (
              ...
              >
                {/* Outer shell */}
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-1.5 backdrop-blur-sm transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:border-white/[0.12]">
                  {/* Inner core */}
        ...
        >
          ...
          {/* ── Satellite Hubs - Double-bezel cards ── */}
          {GLOBAL_HUBS.filter(h => h.distance > 0).map((hub, i) => {
            const angle = (hub.angle * Math.PI) / 180
            const x = 50 + (hub.distance / 300) * 50 * Math.cos(angle)
            const y = 50 + (hub.distance / 300) * 50 * Math.sin(angle)
            return (
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1 }}
                transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1], delay: 0.6 + i * 0.15 }}
              ...
              >
                {/* Outer shell */}
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-1 backdrop-blur-sm transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:border-cyan-400/20 group-hover:bg-white/[0.04]">
                  {/* Inner core */}
          ...
          {/* ── Ambient glow pulses ── */}
          <div className="absolute left-1/2 top-1/2 h-40 w-40 -translate-x-1/2 -translate-y-1/2 rounded-full bg-cyan-500/10 blur-3xl animate-pulse" />
        </motion.div>
      </div>
