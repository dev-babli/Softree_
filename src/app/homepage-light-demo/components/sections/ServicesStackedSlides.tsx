...
export function ServicesStackedSlides({ className = "" }: { className?: string }) {
   ...
   *
   * Section pin span (sticky height) is capped at 50svh per slide on
   * desktop to satisfy Requirement 6.7 (no pin > 50% of viewport height).
   */

  return (
    <section
      ref={rootRef}
      data-section="services-stack"
      ...
      <div className="ssx-container">
        ...
        <div className="ssx-slides-wrapper">
          {SERVICE_SLIDES.map((slide, i) => (
            <section
              key={slide.key}
              className={`ssx-section ssx-section-${slide.key}${i % 2 === 1 ? " ssx-section--reverse" : ""}`}
            >
