...
export default function IndustryCard({ item, isActive }: { item: any, isActive: boolean }) {
  return (
    <div 
      className={`relative flex h-auto min-h-[360px] w-[min(340px,calc(100vw-2.5rem))] max-w-[340px] flex-col overflow-hidden rounded-[28px] bg-white transition-all duration-300 sm:h-[400px] sm:min-h-[400px] group ${isActive ? 'shadow-[0_20px_40px_rgba(255,107,44,0.15)] border-2 border-[#FF6B2C]' : 'shadow-[0_10px_30px_rgba(0,0,0,0.06)] border border-[#ECECEC]'}`}
    >
      {/* Top Image Placeholder */}
      <div className={`relative h-[160px] w-full overflow-hidden bg-gradient-to-br sm:h-[180px] ${item.color} transition-transform duration-700 ${isActive ? 'scale-100' : 'scale-105'}`}>
         {(item.image || imageMap[item.icon]) ? (
           <Image 
             src={item.image || imageMap[item.icon]}
      ...
      {/* Floating Icon */}
      <div className={`absolute left-6 top-[136px] z-20 flex h-12 w-12 items-center justify-center rounded-full bg-white text-[#FF6B2C] shadow-md transition-colors duration-300 sm:top-[156px]`}>
      ...
      <div className="pt-10 px-6 pb-6 flex-1 flex flex-col relative bg-white z-10">
         ...
         <p className="line-clamp-5 flex-1 text-[14px] leading-relaxed text-[#6B7280]">{item.description}</p>
      </div>
    </div>
  );
}
