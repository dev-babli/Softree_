...
import { documentAiCapabilitiesData } from './documentAiCapabilitiesData';

export default function DocumentAiCapabilities() {
  const [activeIndex, setActiveIndex] = useState(3); // Default to 04 (index 3) as requested
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % documentAiCapabilitiesData.length);
    }, 2500);
  ...
  const activeData = documentAiCapabilitiesData[activeIndex];

  return (
    <section className="relative w-full pt-4 pb-10 lg:pt-4 lg:pb-14 bg-transparent overflow-hidden font-sans">
      ...
      <div className="relative z-10 max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center w-full mb-8 lg:mb-10">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-[2.5rem] font-extrabold text-[#111827] mb-3 tracking-tight text-center leading-tight">
            Enterprise <span className="text-[#FF6B2C]">Document AI Solutions Capabilities</span>
          </h2>

          <p className="text-[15px] lg:text-[17px] text-[#6B7280] text-center max-w-2xl leading-snug">
            Partner with Softree to automate document processing using AI-powered Intelligent Document Processing (IDP), Azure AI Document Intelligence, OCR, and Microsoft AI. We help organizations extract, classify, validate, and manage business documents with greater accuracy, speed, and security.
          </p>
        </div>
        ...
        >
          ...
          <div className="w-full lg:w-[40%] flex flex-col gap-2 lg:gap-3 relative z-10">
            {documentAiCapabilitiesData.map((cap, idx) => {
              ...
              >
                ...
                >
                  {activeData.kpis.map((kpi, i) => (
                    ...
                    >
                      <div className="text-lg lg:text-xl xl:text-2xl font-bold text-[#FF6B2C] mb-1 group-hover/kpi:scale-105 transition-transform duration-300 w-full px-2 break-words">
                        {kpi.value}
                      </div>
