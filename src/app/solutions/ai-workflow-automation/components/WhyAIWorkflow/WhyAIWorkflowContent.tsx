...
import { WhyAIWorkflowItem } from "./WhyAIWorkflowData";
import { contentVariant, imageVariant, staggerContainer, listItemVariant } from "./animations";

interface Props {
  item: WhyAIWorkflowItem;
  currentIndex: number;
  totalItems: number;
}

export const WhyAIWorkflowContent: React.FC<Props> = ({ item, currentIndex, totalItems }) => {
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <motion.div 
            ...
            className="w-full md:w-5/12 h-[400px] md:h-[500px] rounded-3xl overflow-hidden relative shadow-2xl shadow-orange-500/5 flex-shrink-0 border border-gray-100"
          >
            <Image
              src={item.image}
              alt={item.title}
