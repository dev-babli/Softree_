      ...
      <div className="mx-auto max-w-[85rem]">
        ...
        <div className="mb-10 flex flex-col items-center text-center">
          ...
          <h2 className="mb-2 text-center text-2xl font-extrabold leading-tight tracking-tight text-[#111827] md:mb-3 md:text-4xl lg:text-[2.25rem]">
            Stack for enterprise{" "}
            <span className="text-[#FF6A13]">Azure OpenAI</span>
          </h2>

          <p className="mx-auto mb-6 max-w-2xl text-center text-[15px] leading-relaxed text-[#6B7280] lg:mb-8 lg:text-base">
            We build production-ready Azure OpenAI solutions using GPT models on Azure, Azure AI Search, Microsoft 365, Power Platform, cloud platforms, and enterprise security technologies.
          </p>
        </div>

        {/* ================= TABS ================= */}
        <div className="mb-10 flex justify-center">
          <div className="flex flex-wrap justify-center gap-3 border-b border-gray-200 md:gap-8">
            {aiTechnologyCategories.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`relative pb-3 text-xs font-medium transition sm:text-sm ${
                  activeTab === tab.id
                    ? "text-orange-600"
