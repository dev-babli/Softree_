...
import CapabilitySectionBadge from './CapabilitySectionBadge';
import { coreCapabilitiesData } from './coreCapabilitiesData';

export default function EnterpriseCapabilitiesBentoGrid() {
  ...
  return (
    <section className="relative w-full py-12 md:py-16 lg:py-20 bg-transparent overflow-hidden font-sans">
      {/* Background Decorators */}
      {/* Top Left */}
      <div className="absolute top-0 left-0 w-100 h-100 border-b border-r border-[#FF6B2C]/10 rounded-br-[100%] opacity-20 pointer-events-none -translate-x-1/4 -translate-y-1/4"></div>
      ...
      <div className="relative z-10 max-w-340 mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center w-full mb-8 lg:mb-10">
          <CapabilitySectionBadge text="CORE CAPABILITIES" variant="line" />
          ...
          <h2 className="text-3xl md:text-4xl lg:text-[2.5rem] font-extrabold text-[#111827] mb-3 tracking-tight text-center leading-tight">
            Enterprise AI <span className="text-[#FF6B2C]">Production Capabilities</span>
          </h2>

          <p className="text-[15px] lg:text-[17px] text-[#6B7280] text-center max-w-2xl leading-snug">
            Partner with Softree to design, build, and secure enterprise-grade AI architectures. From resilient MLOps pipelines to private RAG systems, we engineer solutions that scale.
          </p>
        </div>
        ...
        >
          ...
          {/* Right Showcase (60%) */}
          <div className="w-full lg:w-[60%] lg:pl-2 h-full flex flex-col">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="bg-white rounded-3xl shadow-[0_10px_40px_rgba(255,107,44,0.08)] overflow-hidden-col h-full border border-[#FF6B2C] grow"
              >
                ...
                <div className="flex flex-col md:flex-row grow">
                  ...
                  <motion.div
                    ...
                    className={`w-full md:w-[45%] bg-[#F8F9FB] relative overflow-hidden items-center justify-center ${(activeData as any).image ? '' : 'p-6'} group border-r border-gray-100`}
                  >
                    {/* Abstract Illustration Based on Capability */}
                    <div className="relative z-10 w-full h-full flex items-center justify-center transition-transform duration-700 group-hover:scale-105 min-h-62.5 lg:min-h-75">
                  ...
                  {/* Content Area */}
                  <div className="w-full md:w-[55%] p-5 sm:p-6 lg:p-8 flex flex-col justify-center grow">
                      ...
                      {activeData.highlights.map((highlight, i) => {
                        ...
                        return (
                          <li key={i} className="flex items-start gap-3">
                            <div className="shrink-0 w-10 h-10 rounded-xl bg-white border border-orange-100 flex items-center justify-center text-[#FF6B2C] shadow-sm">
                ...
                >
                  {activeData.kpis.map((kpi, i) => (
                    ...
                    >
                      <div className="text-lg lg:text-xl xl:text-2xl font-bold text-[#FF6B2C] mb-1 group-hover/kpi:scale-105 transition-transform duration-300 w-full px-2 wrap-break-word">
                        {kpi.value}
                      </div>
...
function AbstractIllustration({ type }: { type: string }) {
  if (type === 'strategy') {
    return (
      <svg viewBox="0 0 400 200" className="w-full h-full max-w-75" fill="none" xmlns="http://www.w3.org/2000/svg">
