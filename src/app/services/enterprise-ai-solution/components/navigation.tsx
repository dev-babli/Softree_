...
type SanityNavCaseStudyCategory = {
  title: string;
  description?: string;
  links?: Array<{
    label: string;
...
function MegaMenuPanel({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-black/[0.07] bg-white shadow-[0_22px_60px_rgba(15,23,42,0.14)]">
      <div className="border-b border-black/[0.06] bg-[#fff8f3] px-6 py-3">
        <span className="badge-label text-[#FF5812]">{label}</span>
      </div>
      <div
        className={`grid gap-3 p-4 ${groups.length > 2 ? "grid-cols-3" : groups.length > 1 ? "grid-cols-2" : "grid-cols-1"
          }`}
      >
        {groups.map((group) => (
          <div key={group.title} className="rounded-xl border border-black/[0.06] bg-white p-3">
            <div className="mb-2 px-2">
              <p className="text-[13px] font-semibold text-[#0a0a1a]">{group.title}</p>
              {group.description && (
                <p className="mt-1 text-[11px] leading-relaxed text-[#0a0a1a]/45">
                  {group.description}
                </p>
              )}
            </div>
                ...
                return (
                  <li key={`${group.title}-${link.label}`}>
                    <Link
                      href={link.url}
                      onClick={onClose}
                      className="group flex min-h-12 items-start gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-orange-50"
                    >
                      {Icon && (
                        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-orange-50 text-[#FF5812] group-hover:bg-[#FF5812] group-hover:text-white">
                          <Icon size={16} />
                        </span>
                      )}
                      <span className="min-w-0">
                        <span className="block text-[12px] font-semibold text-[#0a0a1a] group-hover:text-[#FF5812]">
                          {link.label}
                        </span>
                        {link.description && (
                          <span className="mt-0.5 block text-[10px] leading-snug text-[#0a0a1a]/42">
                            {link.description}
                          </span>
                        )}
                      </span>
