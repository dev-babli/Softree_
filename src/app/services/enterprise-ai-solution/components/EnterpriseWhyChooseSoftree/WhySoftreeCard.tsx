...
export const WhySoftreeCard: React.FC<Props> = ({
  ...
  return (
    <div
      ...
      className={`relative w-full sm:w-20 lg:w-24 shrink-0 cursor-pointer transition-all duration-500 ease-in-out flex flex-col items-center justify-between rounded-full overflow-hidden ${
        isActive
          ? "h-125 sm:h-[600px] bg-white shadow-[0_20px_40px_rgba(255,106,19,0.15)] -translate-y-2 z-10"
          : "h-[450px] sm:h-125 bg-white border border-gray-100 hover:shadow-md hover:-translate-y-1 z-0"
      }`}
    >
      {/* Active Glowing Border */}
      {isActive && (
        <>
          <div className="absolute -inset-0.5 rounded-full bg-linear-to-b from-orange-500 via-orange-300 to-orange-500 opacity-70 blur-sm pointer-events-none animate-pulse" />
          <div className="absolute -inset-0.5 rounded-full bg-linear-to-b from-orange-500 via-white to-orange-500 opacity-100 pointer-events-none" style={{ maskImage: "linear-gradient(white, white)", maskComposite: "exclude" }} />
        </>
      )}
