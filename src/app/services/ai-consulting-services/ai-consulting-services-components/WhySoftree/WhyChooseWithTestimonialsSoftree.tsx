...
export default function WhyChooseWithTestimonialsSoftree() {
    const trackRef = useRef<HTMLDivElement>(null);
    const [index, setIndex] = useState(0);
    const [paused, setPaused] = useState(false);
    const [activeMobileTab, setActiveMobileTab] = useState<'why' | 'testimonials'>('why');
        ...
        <section className="text-gray-900 py-12 md:py-16 lg:py-20">
            ...
            <div className="flex lg:hidden justify-center mb-8 px-6">
                <div className="flex bg-orange-50/50 border border-orange-100 p-1.5 rounded-2xl w-full max-w-sm">
                    <button
                        onClick={() => setActiveMobileTab('why')}
                        className={`flex-grow py-2 text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-300 ${
                            activeMobileTab === 'why'
            ...
            <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
                {/* ================= LEFT : WHY CHOOSE ================= */}
                <div className={`relative ${activeMobileTab === 'why' ? 'block' : 'hidden lg:block'}`}>
                    {/* Small Label */}
                ...
                {/* ================= RIGHT : TESTIMONIALS ================= */}
                <div className={`rounded-2xl p-10 bg-gradient-to-r from-black via-[#4c1c02] to-black border border-white/10 shadow-2xl ${activeMobileTab === 'testimonials' ? 'block' : 'hidden lg:block'}`}>
                    {/* Header */}
                    <div className="mb-10">
