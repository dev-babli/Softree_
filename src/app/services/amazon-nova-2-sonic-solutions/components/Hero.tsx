        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-6 xl:gap-8 w-full items-center">
          ...
          <div className="lg:col-span-6 xl:col-span-6 flex flex-col justify-start text-left z-10">
            ...
            <motion.div
              ...
              className="mb-3.5 sm:mb-4 inline-flex items-center gap-2 rounded-full border border-[#FF6B00]/40 bg-[#FF6B00]/10 px-4.5 sm:px-5 py-2 typo-caption text-[#FF6B00] backdrop-blur-md shadow-[0_0_15px_rgba(255,107,0,0.15)] w-fit uppercase"
            >
              <span className="h-2 w-2 rounded-full bg-[#FF6B00] shadow-[0_0_8px_#FF6B00] animate-pulse" />
              <span>AMAZON NOVA 2 SONIC DEVELOPMENT</span>
            </motion.div>
            ...
            >
              <span className="block">Build Real-Time</span>
              <span className="block text-[#FF6B00] drop-shadow-[0_0_25px_rgba(255,107,0,0.4)]">
                Voice AI
              </span>
              <span className="block">With Your</span>
              <span className="block">Offshore Team</span>
            </motion.h1>
...
// --- Three.js Canvas Component with Viewport Culling & Device-Responsive Particle Scaling ---
const WovenCanvas = () => {
  ...
  useEffect(() => {
    ...
    // 2. Camera setup with FOV 55
    const camera = new THREE.PerspectiveCamera(55, aspect, 0.1, 1000);

    // 3. Renderer with antialiasing and transparency (cap pixel ratio to 1.5 for ultra-fast TBT/FID)
    ...
    // 5. Responsive particle count (12k on mobile vs 32k on desktop for 60fps)
    ...
    const torusKnot = new THREE.TorusKnotGeometry(1.20, 0.38, isMobile ? 140 : 260, isMobile ? 32 : 48);
    ...
    const ambientCount = isMobile ? 350 : 800;
    ...
    // 8. Dynamic Camera Distance Calculator
    ...
    // IntersectionObserver to pause rendering when hero is out of view (huge CPU/battery win)
    const intersectionObserver = new IntersectionObserver(([entry]) => {
      isVisible = entry.isIntersecting;
    }, { threshold: 0.05 });
    intersectionObserver.observe(container);

    // 9. Interactive mouse physics
    ...
    // 10. Animation Loop with Pause when off-screen
    ...
    const animate = () => {
      ...
      if (!isVisible) return; // Skip frame rendering entirely when offscreen
  ...
  return <div ref={mountRef} className="w-full h-full relative flex items-center justify-center pointer-events-none sm:pointer-events-auto" />;
};
